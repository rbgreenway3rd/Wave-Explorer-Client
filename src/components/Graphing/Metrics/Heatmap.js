...
//       const slope =
//         filteredData.length > 0 ? linearRegression(filteredData) : 0;
...
import * as d3 from "d3";
import { interpolateRdYlBu, interpolateSpectral, interpolateCool } from "d3";
import { DataContext } from "../../FileHandling/DataProvider";
...
const Heatmap = ({
  ...
  const calculateSlope = (heatmapData) => {
  ...
  const colorScale = useMemo(() => {
    // Define the extent of the values considering negative and positive slopes
    let extent = d3.extent(allValues);

    // If using slopes, we should consider a different scale
    if (metricType === "slope") {
      // Create a diverging color scale
      let extent = [minSlope, maxSlope];
      return d3
        .scaleDiverging()
        .interpolator(
          d3.interpolateRgbBasis([
            // Define your color range for non-slope metrics//
            "#23171b",
            "#271a28",
            "#2b1c33",
            "#2f1e3f",
        ...
        ) // or another diverging color scheme
        .domain([extent[0], 0, extent[1]]); // Use 0 as the midpoint for diverging scale
    }
